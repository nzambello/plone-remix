...
export const loader = async ({ params, request }: LoaderArgs) => {
  const lang = config.settings.isMultilingual
    ? flattenToAppURL(request.url).split('/')?.[1] ?? config.settings.defaultLanguage
    : config.settings.defaultLanguage

  const navReq = await fetch(`${PLONE_RESTAPI_URL}/${lang}/@navigation?expand.navigation.depth=2`, {
    headers: {
  ...
  const content = await plone.content.get(`/${params['*']}?expand=translations`)
  ...
  return (
    <>
      <header>
        ...
        <LanguageSelector currentLang={lang} translations={content['@components'].translations?.items} />
      </header>
      <main>
